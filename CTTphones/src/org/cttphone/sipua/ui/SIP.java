	...
	void callPSTN(String uri) {
		...
		if (uri.indexOf(":") >= 0) {
			...
			if (!number.equals("")) {
				...
				{
					  formNumber = Caller.pre_fix + Uri.decode(number);
				}
				else
				{
					  formNumber = Uri.decode(number);
				}
				
		        Intent intent = new Intent(Intent.ACTION_CALL,
		                Uri.fromParts(Uri.decode(number).contains("@")?"sipdroid":"tel",formNumber, null));
