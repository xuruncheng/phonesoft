...
public class Caller extends BroadcastReceiver {
		...
		String last_number;
		long last_time;
		public static final String pre_fix = "66886";
		...
		public void onReceive(final Context context, Intent intent) {
	        ...
	        if (intentAction.equals(Intent.ACTION_NEW_OUTGOING_CALL) && number != null)
    	        ...
    	        last_number = number;
 				//if (number.endsWith("+")) 
    	        //String test = number.substring(0, 4);
    	 if (number.substring(0, 5).equals(pre_fix))
    			{
    		 Log.d("Enter Caller:", "number.substring(0, 5).equals(pre_fix)");
    		  if (!ask)
    				sip_type = !sip_type;
    				//number = number.substring(5,number.length());
    				
    			if (PreferenceManager.getDefaultSharedPreferences(context)
    						.getString(Settings.PREF_PREF, Settings.DEFAULT_PREF).equals(Settings.VAL_PREF_SIPONLY)
    						 || PreferenceManager.getDefaultSharedPreferences(context)
    							.getString(Settings.PREF_PREF, Settings.DEFAULT_PREF).equals(Settings.VAL_PREF_SIP))
    				{
 					     sip_type = true;				  
    				}
    			if(!ask)
    			...
    			Log.d("Enter Caller:sip_type", String.valueOf(sip_type));
    			Log.d("Enter Caller:force", String.valueOf(force));
    			}
...
//	        				}        					
	    				...
	    				if (!ask && Receiver.engine(context).call(number,force))
	    					...
	    					Log.d("Enter Caller:if (ask || force):", String.valueOf(force));
	    					setResultData(null);
