...
package org.cttphone.sipua;

import java.io.IOException;
import java.net.UnknownHostException;

import org.cttphone.net.KeepAliveSip;
import org.cttphone.sipua.ui.ChangeAccount;
import org.cttphone.sipua.ui.LoopAlarm;
import org.cttphone.sipua.ui.Receiver;
import org.cttphone.sipua.ui.Settings;
...
public class SipdroidEngine implements RegisterAgentListener {
	...
	public boolean StartEngine() {
			...
			if (wl == null) {
				if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).contains(org.cttphone.sipua.ui.Settings.PREF_KEEPON)) {
					Editor edit = PreferenceManager.getDefaultSharedPreferences(getUIContext()).edit();
	
					edit.putBoolean(org.cttphone.sipua.ui.Settings.PREF_KEEPON, true);
					edit.commit();
				}
			...
			for (UserAgentProfile user_profile : user_profiles) {
				if (wl[i] == null) {
					...
					if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(org.cttphone.sipua.ui.Settings.PREF_KEEPON, org.cttphone.sipua.ui.Settings.DEFAULT_KEEPON)) {
	...
	//\B4撕\AF\CA\FD\B5墓\A6\C4\DC为\B7\B4注\B2幔�A3\BF\A3\BF\A3\BF\A3\BF
	...
	public boolean call(String target_url,boolean force) {
		int p = pref;
		boolean found = false;
		Log.d("enter SipdroidEngine.java:call", String.valueOf(force));
		...
		Log.d("enter SipdroidEngine.java:target_url before:", target_url);
		String[] target_url_array = target_url.split("\\s");
		String final_target_url = "";
		if (target_url_array.length > 1)
		{
		  for(int i = 0; i < target_url_array.length; i++)
		     {
			  final_target_url += target_url_array[i];
		     }
		}
		else
		{
		...
		Log.d("enter SipdroidEngine.java:4target_url after:", final_target_url);
		return ua.call(final_target_url, false);
	}
